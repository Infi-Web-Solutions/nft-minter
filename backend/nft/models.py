...
class NFT(models.Model):
    ...
    # New fields for NFT management
    is_burned = models.BooleanField(default=False)  # On-chain burned status
    is_hidden = models.BooleanField(default=False)  # Hidden from marketplace
    burned_at = models.DateTimeField(null=True, blank=True)  # When it was burned
    hidden_at = models.DateTimeField(null=True, blank=True)  # When it was hidden
    hidden_reason = models.CharField(max_length=255, null=True, blank=True)  # Reason for hiding
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    ...
    def can_be_burned(self):
        """Check if NFT can be burned (only by creator, never sold)"""
        return (
            not self.is_burned and 
            self.owner_address == self.creator_address and
            not self.is_listed
        )
    
    @property
    def can_be_hidden(self):
        """Check if NFT can be hidden (by creator or admin)"""
        return not self.is_burned and not self.is_hidden
    
    @property
    def is_visible(self):
        """Check if NFT should be visible in marketplace"""
        return not self.is_burned and not self.is_hidden
